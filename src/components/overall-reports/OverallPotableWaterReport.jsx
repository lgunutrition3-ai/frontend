// components/overall-reports/OverallPotableWaterReport.jsx
...
const OverallPotableWaterReport = () => {
  ...
  const fetchOverallReport = async () => {
    ...
    setError('')
    try {
      const allRecords = await potableWaterApi.getAll()
      ...
      const barangayReports = barangays.map(barangay => {
        ...
        return {
          barangay,
          level1: barangayRecords.filter(r => r.level1 > 0).length,
          level2: barangayRecords.filter(r => r.level2 > 0).length,
          level3: barangayRecords.filter(r => r.level3 > 0).length,
          total: barangayRecords.length
        }
      })
      
      const overallTotal = {
        level1: barangayReports.reduce((sum, b) => sum + b.level1, 0),
        level2: barangayReports.reduce((sum, b) => sum + b.level2, 0),
        level3: barangayReports.reduce((sum, b) => sum + b.level3, 0),
  ...
  const handleExportDocx = async () => {
    ...
    body.push([
      '', 'Total',
      report.overallTotal?.level1 || 0,
      report.overallTotal?.level2 || 0,
      report.overallTotal?.level3 || 0,
    ...
    await exportReportToDocx({
      ...
      titleLines: [`POTABLE WATER OVERALL REPORT ${yearDisplay}`],
      infoLines,
      infoCenter: true,
      headers: ['#', 'BARANGAY', 'LEVEL 1', 'LEVEL 2', 'LEVEL 3', 'TOTAL'],
      body,
      cellFontSize: 18,
      signatures: {
      ...
      fileName: `Overall_PotableWater_Report_${yearDisplay}`
    })
  }
    ...
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div className="d-flex align-items-center gap-3">
          ...
          <div>
            <h1 className="mb-0">Potable Water Overall Report</h1>
      ...
      <Row className="g-4 mb-4">
        ...
        <Col md={3}>
          <Card className="text-center border-0 shadow-sm">
            <Card.Body>
              <h6 className="text-muted">Level 1</h6>
              <h2 className="mb-0 text-info">{report.overallTotal?.level1 || 0}</h2>
            </Card.Body>
          </Card>
        </Col>
        <Col md={3}>
          <Card className="text-center border-0 shadow-sm">
            <Card.Body>
              <h6 className="text-muted">Level 2</h6>
              <h2 className="mb-0 text-success">{report.overallTotal?.level2 || 0}</h2>
            </Card.Body>
          </Card>
        </Col>
        <Col md={3}>
          <Card className="text-center border-0 shadow-sm">
            <Card.Body>
              <h6 className="text-muted">Level 3</h6>
              <h2 className="mb-0 text-warning">{report.overallTotal?.level3 || 0}</h2>
            </Card.Body>
          </Card>
        </Col>
      </Row>
        ...
        <Card.Header className="text-center bg-white border-0 pt-3">
          <div className="d-flex align-items-center justify-content-center gap-3">
            ...
            <div>
              ...
              <div className="fw-bold fs-5 text-success">POTABLE WATER OVERALL REPORT {yearDisplay}</div>
              {startDate && endDate && (
            ...
            <tbody>
              {report.barangays.map((barangay, i) => (
                <tr key={barangay.barangay}>
                  ...
                  <td className="text-center">{barangay.level1 || 0}</td>
                  <td className="text-center">{barangay.level2 || 0}</td>
                  <td className="text-center">{barangay.level3 || 0}</td>
                  <td className="text-center">{barangay.total || 0}</td>
                </tr>
              ))}
              <tr className="table-primary fw-bold">
                <td colSpan={2}>Total</td>
                <td className="text-center">{report.overallTotal?.level1 || 0}</td>
                <td className="text-center">{report.overallTotal?.level2 || 0}</td>
                <td className="text-center">{report.overallTotal?.level3 || 0}</td>
...
export default OverallPotableWaterReport