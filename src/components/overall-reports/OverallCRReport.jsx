// components/overall-reports/OverallCRReport.jsx
...
const OverallCRReport = () => {
  ...
  const fetchOverallReport = async () => {
    ...
    setError('')
    try {
      const allRecords = await crApi.getAll()
      ...
      const barangayReports = barangays.map(barangay => {
        ...
        return {
          barangay,
          withCR: barangayRecords.filter(r => r.withCR === true).length,
          withoutCR: barangayRecords.filter(r => r.withoutCR === true).length,
          total: barangayRecords.length
        }
      })
      
      const overallTotal = {
        withCR: barangayReports.reduce((sum, b) => sum + b.withCR, 0),
        withoutCR: barangayReports.reduce((sum, b) => sum + b.withoutCR, 0),
  ...
  const handleExportDocx = async () => {
    ...
    body.push([
      '', 'Total',
      report.overallTotal?.withCR || 0,
      report.overallTotal?.withoutCR || 0,
    ...
    await exportReportToDocx({
      ...
      titleLines: [`CR OVERALL REPORT ${yearDisplay}`],
      infoLines,
      infoCenter: true,
      headers: ['#', 'BARANGAY', 'WITH CR', 'WITHOUT CR', 'TOTAL'],
      body,
      cellFontSize: 18,
      signatures: {
      ...
      fileName: `Overall_CR_Report_${yearDisplay}`
    })
  }
    ...
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div className="d-flex align-items-center gap-3">
          ...
          <div>
            <h1 className="mb-0">CR Overall Report</h1>
      ...
      <Row className="g-4 mb-4">
        ...
        <Col md={3}>
          <Card className="text-center border-0 shadow-sm">
            <Card.Body>
              <h6 className="text-muted">With CR</h6>
              <h2 className="mb-0 text-success">{report.overallTotal?.withCR || 0}</h2>
            </Card.Body>
          </Card>
        </Col>
        <Col md={3}>
          <Card className="text-center border-0 shadow-sm">
            <Card.Body>
              <h6 className="text-muted">Without CR</h6>
              <h2 className="mb-0 text-danger">{report.overallTotal?.withoutCR || 0}</h2>
            </Card.Body>
          </Card>
        </Col>
        <Col md={3}>
            ...
            <tbody>
              {report.barangays.map((barangay, i) => (
                <tr key={barangay.barangay}>
                  ...
                  <td className="text-center">{barangay.withCR || 0}</td>
                  <td className="text-center">{barangay.withoutCR || 0}</td>
                  <td className="text-center">{barangay.total || 0}</td>
                </tr>
              ))}
              <tr className="table-primary fw-bold">
                <td colSpan={2}>Total</td>
                <td className="text-center">{report.overallTotal?.withCR || 0}</td>
                <td className="text-center">{report.overallTotal?.withoutCR || 0}</td>
...
export default OverallCRReport