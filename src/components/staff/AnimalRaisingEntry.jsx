...
const AnimalRaisingEntry = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = {
        ...
        otherMale: parseInt(formData.otherMale) || 0,
        otherFemale: parseInt(formData.otherFemale) || 0,
        year: year,
        recordedDate: formData.recordedDate
      }

      if (editingId) {
        await animalRaisingApi.update(editingId, data)
        setSuccess('Record updated successfully!')
      } else {
        await animalRaisingApi.create(data)
  ...
  const handleEdit = (record) => {
    ...
    setFormData({
      ...
      otherMale: record.otherMale,
      otherFemale: record.otherFemale,
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      await animalRaisingApi.deleteMany(selectedIds)
      setSelectedIds([])
      fetchRecords()
    } catch (error) {
      ...
      <Card className="mb-4" ref={formCardRef}>
        ...
        <Card.Body>
          ...
          <GuidePanel open={showGuide}>
            <strong>Animal Raising Report</strong>
            <ul className="mb-0 ps-3">
              <li><strong>Purok, Household Name, Record Date</strong> — all required. Household names autocomplete from existing records.</li>
              <li><strong>Animals</strong> — enter how many male (M) and female (F) animals the household is currently raising: Chicken, Pig, Goat, Cow, Carabao, and Other.</li>
              <li>Counts must be whole numbers, zero or higher — negative values are not accepted. Leave as 0 if the household raises none.</li>
            </ul>
          </GuidePanel>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <h6 className="mt-3 mb-3">Animals</h6>
            <Row>
              <Col md={2}>
                <Form.Group className="mb-3">
            ...
            <Row>
              ...
              <Col md={2}>
                <Form.Group className="mb-3">
                  <Form.Label>Other M</Form.Label>
                  <Form.Control
                    type="number"
                    min="0"
                    value={formData.otherMale}
                    onChange={(e) => handleNonNegativeInput(e, 'otherMale')}
                    onKeyDown={(e) => {
                      if (e.key === '-' || e.key === 'e') {
                        e.preventDefault()
                      }
              ...
              <Col md={2}>
                <Form.Group className="mb-3">
                  <Form.Label>Other F</Form.Label>
                  <Form.Control
                    type="number"
                    min="0"
                    value={formData.otherFemale}
                    onChange={(e) => handleNonNegativeInput(e, 'otherFemale')}
                    onKeyDown={(e) => {
                      if (e.key === '-' || e.key === 'e') {
                        e.preventDefault()
                      }
...
export default AnimalRaisingEntry
